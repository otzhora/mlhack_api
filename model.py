...
def load_image(path):
    #####
    img = Image.open(path)
    #img = img.resize((160 * 2, 90 * 2), Image.ANTIALIAS)
    img = img.resize((478, 360), Image.ANTIALIAS)

    return np.array(img)


weights_dir = f'./weights/srgan'
...
def superize(img_arr):
    gan_sr = resolve_single(gan_generator, img_arr)
    return gan_sr  # np.array
